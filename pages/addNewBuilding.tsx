...
export const AddNewBuildingPage: React.FC<Props> = ({ onSaveComplete }) => {
  ...
  const handleFormChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name.startsWith('resourceEfficiency.')) {
        ...
        setBuildingData(prev => ({ ...prev, resourceEfficiency: { ...prev.resourceEfficiency, [key]: isCheckbox ? (e.target as HTMLInputElement).checked : value } }));
    } else if (name === 'roomsMin' || name === 'roomsMax' || name === 'elevators') {
        setBuildingData(prev => ({...prev, [name]: parseInt(value, 10) || 0}));
    }
    else {
        setBuildingData(prev => ({ ...prev, [name]: isCheckbox ? (e.target as HTMLInputElement).checked : value }));
    }
  };
        ...
        return (
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="additionalFeatures" name="additionalFeatures" value={buildingData.additionalFeatures} onChange={handleFormChange} className={commonInputClasses} rows={8} placeholder="e.g., Green roof, high-efficiency windows, smart HVAC..."></textarea>
                </div>
            </div>
        );
      case 5: // Review
        return (
            <div className="space-y-4">
                ...
                <div className="text-sm space-y-2 p-4 bg-slate-100 dark:bg-slate-700/50 rounded-lg">
                    <p><strong>Region:</strong> {buildingData.locationRegion || 'Not set'}</p>
                    <p><strong>Type:</strong> {buildingData.housingType || 'Not set'}, {buildingData.floors} floors</p>
